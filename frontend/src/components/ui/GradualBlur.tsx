...
type ResponsiveSizeKey = 'height' | 'width';
type ResponsiveVariantKey<K extends ResponsiveSizeKey> =
  | `mobile${Capitalize<K>}`
  | `tablet${Capitalize<K>}`
  | `desktop${Capitalize<K>}`;
...
type GradualBlurConfig = Required<Omit<GradualBlurProps, 'children'>> & {
...
const PRESETS: Record<Preset, Partial<GradualBlurConfig>> = {
...
const mergeConfigs = (
  ...configs: Partial<GradualBlurConfig>[]
): Partial<GradualBlurConfig> => {
...
const useResponsiveDimension = <K extends ResponsiveSizeKey>(
  responsive: boolean | undefined,
  config: Partial<GradualBlurConfig>,
  key: K
): GradualBlurConfig[K] | undefined => {
  const [val, setVal] = useState<GradualBlurConfig[K] | undefined>(config[key]);
  useEffect(() => {
    if (!responsive) return;
    const calc = () => {
      const w = window.innerWidth;
      let v: GradualBlurConfig[K] | undefined = config[key];
      const cap = key.charAt(0).toUpperCase() + key.slice(1) as Capitalize<K>;
      const mobileKey = `mobile${cap}` as ResponsiveVariantKey<K>;
      const tabletKey = `tablet${cap}` as ResponsiveVariantKey<K>;
      const desktopKey = `desktop${cap}` as ResponsiveVariantKey<K>;
      if (w <= 480 && config[mobileKey]) v = config[mobileKey] as GradualBlurConfig[K];
      else if (w <= 768 && config[tabletKey]) v = config[tabletKey] as GradualBlurConfig[K];
      else if (w <= 1024 && config[desktopKey]) v = config[desktopKey] as GradualBlurConfig[K];
      setVal(v);
    };
